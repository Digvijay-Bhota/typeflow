...
import { createOrganization, requireOrganizationRole } from "@/server/services/organization.service";
...
vi.mock("@/server/db", () => ({
  db: {
    ...
    organizationMember: { create: vi.fn(), findUnique: vi.fn(), findMany: vi.fn(), update: vi.fn(), delete: vi.fn() },
    assessment: { create: vi.fn(), findUnique: vi.fn(), findMany: vi.fn(), update: vi.fn() },
...
describe("B2B Assessment Services", () => {
  ...
  it("should create an organization", async () => {
    ...
    (db.organization.create as any).mockResolvedValue({ id: "org1", name: "Test Org", slug: "test-org" });
  ...
  it("should create an assessment", async () => {
    (db.organizationMember.findUnique as any).mockResolvedValue({ role: "OWNER", organization: { id: "org1" } });
