...
import { getRedisClient, executeRateLimitScript, generateRateLimitKey } from '../../src/server/lib/redis';
...
describe('Distributed Rate Limiter', () => {
  ...
  describe('2. Atomic concurrency test', () => {
    it('handles 200 concurrent requests with limit 50 perfectly', async () => {
      ...
      const promises = Array.from({ length: requests }).map(() => rateLimit(id, limit, 10000));
        ...
        return new Promise<number>((resolve, reject) => {
          ...
          const worker = new Worker(workerCode, {
            ...
            workerData: {
              id: testId,
              limit: limitVal,
              rateLimitPath: path.resolve(__dirname, '../../src/server/middleware/rateLimit.ts')
            },
