...
describe("B2B PostgreSQL Pagination Tests", () => {
  ...
  beforeAll(async () => {
    ...
    orgA = await db.organization.create({ data: { name: `Org A ${suffix}`, slug: `org-a-${suffix}`, ownerId: owner.id } });
    orgB = await db.organization.create({ data: { name: `Org B ${suffix}`, slug: `org-b-${suffix}`, ownerId: owner.id } });

    await db.organizationMember.create({ data: { orgId: orgA.id, userId: owner.id, role: "OWNER" } });
    await db.organizationMember.create({ data: { orgId: orgB.id, userId: owner.id, role: "OWNER" } });

    assessmentA = await db.assessment.create({
      data: { orgId: orgA.id, title: "Assessment A", testMode: "TIMED", language: "ENGLISH", duration: 60, status: "PUBLISHED" }
    });
    
    assessmentB = await db.assessment.create({
      data: { orgId: orgB.id, title: "Assessment B", testMode: "TIMED", language: "ENGLISH", duration: 60, status: "PUBLISHED" }
    });
  ...
  afterAll(async () => {
    await db.assessmentCandidate.deleteMany({ where: { assessmentId: { in: [assessmentA.id, assessmentB.id] } } });
  ...
  it("Case D: Cross-organization request isolation", async () => {
    ...
    const dbAssessment = await db.assessment.findUnique({ where: { id: orgB_assessment } });
