...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ error?: string, claimToken?: string, claimed?: string }> }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] px-4 animate-fade-in">
      <div className="w-full max-w-md bg-background dark:bg-background border border-border dark:border-border rounded-2xl p-8">
        <h1 className="text-2xl font-bold text-center mb-6 text-foreground dark:text-foreground">Sign In</h1>
        ...
        <div className="mt-6 text-center text-sm text-muted">
          Don&apos;t have an account? <Link href={params.claimToken ? `/signup?claimToken=${params.claimToken}` : "/signup"} className="text-accent hover:underline">Sign up</Link>
        </div>
      </div>
