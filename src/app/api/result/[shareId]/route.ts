...
export async function GET(req: Request, { params }: { params: Promise<{ shareId: string }> }) {
  try {
    ...
    if (!shareId || typeof shareId !== "string") {
      return NextResponse.json({ error: { requestId: crypto.randomUUID(), code: "VALIDATION_ERROR", message: "Invalid shareId." } }, { status: 400 });
    }
    ...
    if (!result) {
      return NextResponse.json({ error: { requestId: crypto.randomUUID(), code: "NOT_FOUND", message: "Result not found." } }, { status: 404 });
    }

    return NextResponse.json(result, { status: 200 });
  } catch (error: any) { // eslint-disable-line @typescript-eslint/no-explicit-any
    logger.error("Failed to retrieve result", { error: error.message });
    return NextResponse.json({ error: { requestId: crypto.randomUUID(), code: "INTERNAL_ERROR", message: "Failed to retrieve result." } }, { status: 500 });
  }
}
