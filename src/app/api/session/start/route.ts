...
export async function POST(req: Request) {
  try {
    ...
    if (error.message.includes("Session not found") || error.message.includes("Session is already") || error.message.includes("expired")) {
      return NextResponse.json({ error: { requestId: crypto.randomUUID(), code: "INVALID_SESSION", message: error.message } }, { status: 400 });
    }

    return NextResponse.json({ error: { requestId: crypto.randomUUID(), code: "INTERNAL_ERROR", message: "Failed to start session." } }, { status: 500 });
  }
}
