...
import { Keyboard, Activity, Target, Award, ArrowRight, Flame, Code2, Clock, CheckCircle2, AlertTriangle } from "lucide-react";
import {
  XAxis,
  YAxis,
  Tooltip,
  ...
  if (!stats || stats.totalTests === 0) {
    return (
      <div className="animate-fade-in space-y-8">
        ...
        <div className="opacity-50 mt-12 space-y-8 pointer-events-none filter grayscale transition-all duration-1000">
           <h3 className="text-xl font-semibold text-center mb-8">What you&apos;ll unlock</h3>
           <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
             {[1,2,3,4].map(i => (
                <div key={i} className="bg-surface border-border border rounded-2xl p-6 h-32 blur-[2px]" />
             ))}
           </div>
           <div className="bg-surface border-border border rounded-2xl p-6 h-64 blur-[3px]" />
        </div>
      </div>
    );
  }
    ...
    <div className="animate-fade-in space-y-8 pb-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          <Link
            href="/"
            className="bg-accent text-accent-foreground hover:bg-accent/90 flex items-center gap-2 rounded-full px-6 py-2.5 font-bold transition-all shadow-lg shadow-accent/20"
          >
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        ...
        <div className="bg-surface border-border rounded-2xl border p-5 shadow-sm transition-transform hover:-translate-y-1">
          ...
          <div className="text-3xl font-black">{Math.round(stats.avgAccuracy * 100)}%</div>
        </div>
        <div className="bg-surface border-border rounded-2xl border p-5 shadow-sm transition-transform hover:-translate-y-1">
          <div className="text-muted mb-3 flex items-center gap-2 text-sm font-medium">
            <Award className="h-4 w-4" /> Personal Best
          </div>
          <div className="text-3xl font-black text-emerald-500">{Math.round(stats.maxWpm)}</div>
        </div>
        <div className="bg-surface border-border rounded-2xl border p-5 shadow-sm transition-transform hover:-translate-y-1">
          <div className="text-muted mb-3 flex items-center gap-2 text-sm font-medium">
            <Flame className="h-4 w-4 text-orange-500" /> Current Streak
          </div>
          <div className="text-3xl font-black text-orange-500">{stats.currentStreak} <span className="text-base text-muted font-normal">days</span></div>
        </div>
      </div>

      <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
        {/* Main Chart */}
        <div className="bg-surface border-border lg:col-span-2 rounded-3xl border p-6 shadow-sm">
          <div className="mb-8 flex items-center justify-between">
            ...
            <select 
              ...
              className="bg-background border-border focus:ring-accent rounded-lg border px-3 py-1.5 text-sm outline-none focus:ring-1 cursor-pointer"
            >
          ...
          <div className="h-[320px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                />
                <Area type="monotone" dataKey="wpm" name="WPM" stroke="var(--accent)" strokeWidth={3} fillOpacity={1} fill="url(#colorWpm)" activeDot={{ r: 6, strokeWidth: 0, fill: "var(--accent)" }} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <div className="bg-surface border-border rounded-3xl border p-6 shadow-sm">
            ...
            {stats.weakKeys && stats.weakKeys.length > 0 ? (
              <div className="space-y-3">
                <p className="text-sm text-muted mb-4">Keys you miss most often:</p>
                {stats.weakKeys.map((wk: any, i: number) => (
                  <div key={i} className="flex items-center justify-between bg-background border border-border p-3 rounded-xl">
                    <kbd className="bg-surface-elevated px-3 py-1 rounded font-mono text-lg font-bold border border-border">{wk.key}</kbd>
          ...
          <div className="bg-surface border-border rounded-3xl border p-6 shadow-sm">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center py-2 border-b border-border/50">
                <span className="text-muted text-sm flex items-center gap-2"><Keyboard className="h-4 w-4" /> Total Tests</span>
                <span className="font-bold">{stats.totalTests}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-border/50">
                <span className="text-muted text-sm flex items-center gap-2"><Clock className="h-4 w-4" /> Time Typing</span>
                <span className="font-bold">{formatTime(stats.totalTimeMs)}</span>
              </div>
              <div className="flex justify-between items-center py-2">
                <span className="text-muted text-sm flex items-center gap-2"><Activity className="h-4 w-4" /> Keystrokes</span>
                <span className="font-bold">{stats.totalKeystrokes.toLocaleString()}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-surface border-border overflow-hidden rounded-3xl border shadow-sm">
        <div className="border-border flex items-center justify-between border-b p-6 bg-surface-elevated/30">
          <h2 className="text-xl font-bold">Recent Tests</h2>
          <Link href="/dashboard/history" className="text-accent text-sm font-bold hover:underline bg-accent/10 px-4 py-2 rounded-full transition-colors hover:bg-accent/20">
            View full history
          </Link>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead className="bg-background/50 text-muted text-xs font-semibold uppercase tracking-wider">
              <tr>
            ...
            <tbody className="divide-border divide-y">
              {stats.recentResults.slice(0, 8).map((r: any) => (
                <tr key={r.id} className="hover:bg-background/80 transition-colors group">
                  <td className="text-muted px-6 py-4 whitespace-nowrap font-medium">
                  ...
                  <td className="px-6 py-4">
                    {r.session.language === "CODE" ? (
                      <span className="text-blue-400 flex items-center gap-1 font-mono text-xs font-bold bg-blue-500/10 px-2 py-1 rounded w-fit border border-blue-500/20">
                        <Code2 className="h-3 w-3" /> {r.session.codeLanguage || "CODE"}
                      </span>
                    ) : (
                      <span className="text-muted font-medium capitalize">{r.session.language.toLowerCase()}</span>
                    )}
                  </td>
                  <td className="px-6 py-4 font-black text-right text-base">{Math.round(r.wpm)}</td>
                  <td className="px-6 py-4 text-right font-medium">{Math.round(r.accuracy * 100)}%</td>
