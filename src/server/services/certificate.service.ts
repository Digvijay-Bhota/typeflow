...
import { CERTIFICATE_MIN_WPM, CERTIFICATE_MIN_ACCURACY, CERTIFICATE_MIN_DURATION, CERTIFICATE_ID_PREFIX } from "@/lib/constants";
...
export async function checkCertificateEligibility(resultId: string): Promise<EligibilityResult> {
  ...
  if (!wpmPassed) reasons.push(`Net WPM below minimum (${result.netWpm.toFixed(1)} < ${CERTIFICATE_MIN_WPM})`);
  if (!accPassed) reasons.push(`Accuracy below minimum (${(result.accuracy * 100).toFixed(1)}% < ${CERTIFICATE_MIN_ACCURACY}%)`);
  if (!durationPassed) reasons.push(`Test duration below minimum (${result.session.duration || 0}s < ${CERTIFICATE_MIN_DURATION}s)`);
  if (!isVerified) reasons.push(`Result integrity is ${result.integrityStatus}, must be VERIFIED`);
  ...
  const eligible = wpmPassed && accPassed && durationPassed && isVerified && isCertMode && !!result.userId;
  ...
  return {
    ...
    thresholdSummary: {
      ...
      accuracy: { actual: result.accuracy * 100, required: CERTIFICATE_MIN_ACCURACY, passed: accPassed },
      duration: { actual: result.session.duration || 0, required: CERTIFICATE_MIN_DURATION, passed: durationPassed }
    }
  };
}

export function generateVerificationHash(certificateId: string, userId: string, issuedAt: Date): string {
...
export async function generateCertificatePdfAndQr(certificateId: string) {
   ...
   page.drawText("TypeFlow", { x: 50, y: 500, size: 40, font: fontBold, color: rgb(0.2, 0.4, 0.8) });
   page.drawText("Verified Typing Performance Certificate", { x: 50, y: 460, size: 24, font });
   ...
   page.drawText(`WPM: ${Math.round(cert.wpm)}`, { x: 400, y: 380, size: 30, font: fontBold });
   page.drawText(`Accuracy: ${(cert.accuracy * 100).toFixed(1)}%`, { x: 400, y: 340, size: 24, font });
   
   page.drawText(`Certificate ID: ${cert.certificateId}`, { x: 50, y: 100, size: 12, font });
   page.drawText(`Issued: ${cert.issuedAt.toISOString().split('T')[0]}`, { x: 50, y: 80, size: 12, font });
   page.drawText("Verified by TypeFlow's server-side typing assessment system.", { x: 50, y: 60, size: 10, font, color: rgb(0.4, 0.4, 0.4) });
   ...
   try {
     ...
     console.error("Storage credentials unavailable or upload failed. Using fallback URL.", error);
...
export async function revokeCertificate(adminUserId: string, certificateId: string, reason: string) {
